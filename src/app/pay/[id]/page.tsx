    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-slate-500 font-medium">Chargement du lien de paiement sécurisé...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900 border border-emerald-500/30 rounded-3xl p-8 text-center shadow-2xl">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Paiement Validé avec Succès !</h1>
          <p className="text-slate-700 text-sm mb-6">
            Votre abonnement <strong className="text-orange-400 font-semibold">{paymentData?.planName}</strong> est immédiatement activé pour <strong className="text-slate-900">{months} mois</strong>. Votre menu digital reste 100% accessible.
          </p>

          <div className="bg-slate-800/80 border border-slate-200 rounded-2xl p-4 text-left text-xs text-slate-700 space-y-2 mb-6">
            <div className="flex justify-between">
              <span className="text-slate-500">Restaurant :</span>
              <span className="font-semibold text-slate-900">{paymentData?.tenantName}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-slate-900">{selectedProvider === 'WAVE' ? 'Wave Sénégal' : 'Orange Money'}</span>
            </div>
            <div className="flex justify-between">
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col items-center justify-center p-4 sm:p-6">
      ...
      <div className="max-w-md w-full bg-white border border-slate-200 shadow-xl rounded-3xl p-6 sm:p-8 shadow-2xl">
        ...
        <div className="bg-gradient-to-br from-amber-500/10 to-orange-500/5 border border-slate-200/80 rounded-2xl p-5 mb-6 text-center relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-orange-500/10 rounded-full blur-xl pointer-events-none"></div>
          <p className="text-xs text-slate-500 uppercase tracking-wider font-bold mb-1">Règlement d'abonnement</p>
          <h2 className="text-xl font-bold text-slate-900 mb-2">{paymentData?.tenantName}</h2>
          <div className="text-3xl font-extrabold text-orange-400 tracking-tight">
            {paymentData?.amount?.toLocaleString('fr-FR')} <span className="text-lg font-bold text-slate-700">FCFA</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center p-4 rounded-2xl border transition-all ${
              selectedProvider === 'WAVE'
                ...
                : 'bg-slate-800/60 border-slate-200 text-slate-500 hover:border-slate-600'
            }`}
          >
            ...
            <span className="font-bold text-sm text-slate-900">Wave</span>
          ...
          >
            ...
            <span className="font-bold text-sm text-slate-900">Orange Money</span>
