...
export const SuperAdminAuthGuard: React.FC<SuperAdminAuthGuardProps> = ({ children }) => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-[url('/login-bg.jpg')] bg-cover bg-center bg-no-repeat relative text-white flex items-center justify-center p-4 sm:p-6 font-sans">
        ...
        <div className="w-full max-w-md backdrop-blur-2xl bg-slate-900/85 border border-white/15 rounded-3xl p-6 sm:p-8 shadow-2xl relative z-10 overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-4 relative z-10">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-950/90 border border-slate-700/80 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/30 rounded-2xl px-4 py-3.5 text-sm text-white placeholder:text-slate-500 outline-none transition-all shadow-inner"
                autoFocus
              />
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-[#FF6B00] via-orange-600 to-[#00A86B] hover:opacity-95 active:scale-[0.98] text-white font-black text-sm py-4 px-4 rounded-2xl shadow-xl shadow-orange-600/30 transition-all flex items-center justify-center gap-2"
            >
              {isLoading ? (
          ...
          <div className="mt-6 pt-4 border-t border-slate-800/80 text-center relative z-10 flex items-center justify-between text-[11px] text-slate-400">
  ...
  return (
    <div>
      ...
      <div className="bg-slate-950 text-slate-400 border-b border-slate-800 px-4 py-1.5 text-xs flex items-center justify-between print:hidden">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span className="font-bold text-slate-200">Portail Agence Super Admin</span>
