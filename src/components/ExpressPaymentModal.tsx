    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl relative text-white max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">Lien Direct Wave & Orange Money</h3>
            <p className="text-xs text-slate-400">Deep Linking mobile & partage WhatsApp</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1">Pack SaaS :</label>
            <select
              value={selectedPlan}
              onChange={(e) => setSelectedPlan(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-3 py-2 text-xs font-medium focus:ring-2 focus:ring-orange-500 outline-none"
            >
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1">Durée :</label>
            <select
              value={months}
        ...
        {/* Amount Box */}
        <div className="bg-slate-800/80 border border-slate-700 rounded-2xl p-4 text-center mb-5">
          <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Total à payer :</span>
        ...
        <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-4 space-y-3">
          <p className="text-xs font-bold text-slate-300">Partager le lien au gérant :</p>
          
          <div className="flex gap-2">
            <button
              onClick={() => copyToClipboard(links?.publicPaymentUrl)}
              className="flex-1 py-2 px-3 bg-slate-700 hover:bg-slate-650 text-slate-200 hover:text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition"
            >
            ...
            <button
              ...
              className="flex-1 py-2 px-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition"
            >
